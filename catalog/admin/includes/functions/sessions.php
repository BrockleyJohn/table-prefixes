...
*/
  ...
  if (defined('DIR_FS_SESSION') && DIR_FS_SESSION && is_dir(DIR_FS_SESSION) && is_writable(DIR_FS_SESSION)) {
    ...
    function _sess_write($key, $value) {
      $check_query = tep_db_query("select 1 from {$GLOBALS['pfx']}sessions where sesskey = '" . tep_db_input($key) . "'");
    ...
    function _sess_destroy($key) {
      $result = tep_db_query("delete from {$GLOBALS['pfx']}sessions where sesskey = '" . tep_db_input($key) . "'");

      return $result !== false;
    }

    function _sess_gc($maxlifetime) {
      $result = tep_db_query("delete from {$GLOBALS['pfx']}sessions where expiry < '" . (time() - $maxlifetime) . "'");
