...
*/
  ...
  if (defined('DIR_FS_SESSION') && DIR_FS_SESSION && is_dir(DIR_FS_SESSION) && is_writable(DIR_FS_SESSION)) {
    ...
    function _sess_read($key) {
      $value_query = tep_db_query("SELECT value FROM {$GLOBALS['pfx']}sessions WHERE sesskey = '" . tep_db_input($key) . "'");
    ...
    function _sess_write($key, $value) {
      return false !== tep_db_query("INSERT INTO {$GLOBALS['pfx']}sessions (sesskey, expiry, value) VALUES ('"
    ...
    function _sess_destroy($key) {
      return false !== tep_db_query("DELETE FROM {$GLOBALS['pfx']}sessions WHERE sesskey = '" . tep_db_input($key) . "'");
    }

    function _sess_gc($maxlifetime) {
      return false !== tep_db_query("DELETE FROM {$GLOBALS['pfx']}sessions WHERE expiry < '" . (time() - $maxlifetime) . "'");
    }
